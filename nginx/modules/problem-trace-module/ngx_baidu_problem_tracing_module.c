...
/**
 * @file ngx_http_baidu_problem_tracing_module.c
...
static ngx_int_t ngx_http_baidu_problem_tracing_process_init(ngx_cycle_t *cycle);
static ngx_int_t ngx_http_get_variable_uniqid(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);


static ngx_int_t ngx_http_get_variable_unknown(ngx_http_request_t *r,ngx_http_variable_value_t *v, uintptr_t data);
static void* ngx_http_baidu_problem_tracing_create_loc_conf(ngx_conf_t *cf);
static char* ngx_http_baidu_problem_tracing_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
...
static ngx_int_t ngx_http_baidu_problem_tracing_add_variables(ngx_conf_t *cf);
static ngx_int_t ngx_http_baidu_problem_tracing_init(ngx_conf_t *cf);
...
static u_char *ngx_http_baidu_problem_tracing_log_error_handler(ngx_http_request_t *r, ngx_http_request_t *sr, u_char *buf, size_t len);
...
static ngx_int_t g_product_index = -1;
static ngx_int_t g_subsys_index = -1;
...
} ngx_http_baidu_problem_tracing_loc_conf_t;

static ngx_command_t  ngx_http_baidu_problem_tracing_commands[] = {
    {
		ngx_string("problem_tracing"),
		NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, enable),
		NULL
    },
    {
		...
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, header_enable),
		NULL
    },
    {
		...
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, body_enable),
		NULL
    },
    {
		...
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, get_header_enable),
		NULL
    },
    {
		...
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, white_page_rewrite_enable),
		NULL
    },
    {
		...
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, logid_name),
		NULL
    },
    {
		...
		ngx_conf_set_str_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, tracecode_name),
		NULL
    },

    {
		...
		ngx_conf_set_str_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, js_trace_page_logid_name),
		NULL
    },

    {
		...
		ngx_conf_set_flag_slot,
		NGX_HTTP_LOC_CONF_OFFSET,
		offsetof(ngx_http_baidu_problem_tracing_loc_conf_t, js_trace_logid_enable),
		NULL
    },
...
static ngx_http_variable_t ngx_http_logid_vars[] = {
...

	{
		ngx_string("product"), 
		0,
		ngx_http_get_variable_unknown, 0,
...
static ngx_http_module_t  ngx_http_baidu_problem_tracing_module_ctx = {
	ngx_http_baidu_problem_tracing_add_variables,                          /* preconfiguration */
	ngx_http_baidu_problem_tracing_init,           /* postconfiguration */
	...
	ngx_http_baidu_problem_tracing_create_loc_conf,  /* create location configuration */
	ngx_http_baidu_problem_tracing_merge_loc_conf /* merge location configuration */
};


ngx_module_t  ngx_http_baidu_problem_tracing_module = {
	NGX_MODULE_V1,
	&ngx_http_baidu_problem_tracing_module_ctx, /* module context */
	ngx_http_baidu_problem_tracing_commands,   /* module directives */
	NGX_HTTP_MODULE,               /* module type */
	NULL,                          /* init master */
	NULL,                          /* init module */
	ngx_http_baidu_problem_tracing_process_init,                          /* init process */
	NULL,                          /* init thread */
	NULL,                          /* exit thread */
	NULL,                          /* exit process */
	NULL,                          /* exit master */
...
static ngx_int_t
ngx_http_baidu_problem_tracing_add_variables(ngx_conf_t *cf)
{
...
{
	ngx_http_baidu_problem_tracing_loc_conf_t *conf = NULL;
	conf = ngx_http_get_module_loc_conf(r, ngx_http_baidu_problem_tracing_module);
...
static ngx_int_t
ngx_http_get_variable_unknown(ngx_http_request_t *r,
		ngx_http_variable_value_t *v, uintptr_t data)
{
	v->len= 0;
	v->valid = 0;
		...
		ngx_http_variable_value_t *v, uintptr_t data)
{
	...
	r->variables[g_is_html_index].data = PT_ORIGINAL;

	return NGX_OK;
}

static ngx_int_t
ngx_http_baidu_problem_tracing_init(ngx_conf_t *cf)
{
	...
	ngx_str_t product = ngx_string("product");
	g_product_index = ngx_http_get_variable_index(cf, &product);

	ngx_str_t subsys = ngx_string("subsys");
	g_subsys_index = ngx_http_get_variable_index(cf, &subsys);
	...
	if(g_logid_index == NGX_ERROR || g_tracecode_index == NGX_ERROR || 
           ...
           g_product_index == NGX_ERROR || g_subsys_index == NGX_ERROR || 
...
ngx_http_tracecode_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	if(!strncmp((char *)(r->variables[g_is_html_index].data), PT_ORIGINAL, 1)) {
		...
		if(pointer != NULL && !strncmp(pointer, "<!DOCTYPE", 9)){
			r->variables[g_is_html_index].data = PT_HTML;
			r->variables[g_is_html_index].len = 1;
		} else {
			r->variables[g_is_html_index].data = PT_NOT_HTML;
			r->variables[g_is_html_index].len = 1;
		}
	}
	...
	if (chain_contains_last_buffer && !strncmp(r->variables[g_is_html_index].data, PT_HTML, 1)) {
...
{
	r->log_handler = ngx_http_baidu_problem_tracing_log_error_handler;

	return NGX_DECLINED;
}

static u_char *
ngx_http_baidu_problem_tracing_log_error_handler(ngx_http_request_t *r, ngx_http_request_t *sr,
    u_char *buf, size_t len)
{
	...
	//»ñÈ¡product
	if(r->variables[g_product_index].data != NULL ) {
		p = ngx_snprintf(buf, len, " product: %v, ", &(r->variables[g_product_index]));
		len -= p - buf;
		buf = p;
	}else{
	...
	if(r->variables[g_subsys_index].data != NULL ) {
		p = ngx_snprintf(buf, len, " subsys: %v, ", &(r->variables[g_subsys_index]));
		len -= p - buf;
		buf = p;
	}
	else {
...
static void *
ngx_http_baidu_problem_tracing_create_loc_conf(ngx_conf_t *cf)
{
	ngx_http_baidu_problem_tracing_loc_conf_t  *conf = NULL;

	conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_baidu_problem_tracing_loc_conf_t));
	if (conf == NULL) {
...
static char *
ngx_http_baidu_problem_tracing_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
	ngx_http_baidu_problem_tracing_loc_conf_t *prev = parent;
	ngx_http_baidu_problem_tracing_loc_conf_t *conf = child;
